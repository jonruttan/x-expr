...
static char *test_gc_mark(void)
{
	...
	p_ret = x_gc_mark(NULL, p_obj[0], X_OBJ_FLAG_2 | X_OBJ_FLAG_3);
...
/*
	...
	p_ret = x_gc_mark(NULL, p_obj[2], 1);
...
/*	p_base = x_obj_alloc(NULL, X_TYPE_SYMBOL, 0, 1);
	...
	p_ret = x_gc_mark(p_base, p_base, 1);
...
static char *test_gc_sweep(void)
{
	...
	_it_should("set the base object's gc pointer to NULL", NULL == x_obj_gc(p_base));

	/* Create an object */
	n = helper_alloc_count();
	p_obj[0] = x_obj_alloc(p_base, NULL, X_OBJ_FLAG_RO|X_OBJ_FLAG_GC, 0);
	_it_should("allocate memory for the object", 1 == helper_alloc_count() - n);
	_it_should("set the object's flags to GC,RO", (X_OBJ_FLAG_RO|X_OBJ_FLAG_GC) == x_obj_flags(p_obj[0]));
	_it_should("set the object's gc pointer to NULL", NULL == x_obj_gc(p_obj[0]));
	_it_should("set the base object's gc pointer to the object", p_obj[0] == x_obj_gc(p_base));

	/* Sweep the RO flag from the object */
	n = helper_free_count();
	p_ret = x_gc_sweep(p_base, p_obj[0], X_OBJ_FLAG_RO);
	_it_should("not have freed the object's memory", 0 == helper_free_count() - n);
	_it_should("return the base object", p_base == p_ret);
	_it_should("cleared RO bit in the object's flags", X_OBJ_FLAG_GC == x_obj_flags(p_obj[0]));

	/* Garbage collect everything */
	n = helper_free_count();
	p_ret = x_gc_sweep(p_base, p_base, X_OBJ_FLAG_NONE);
	...
	_it_should("allocate memory for the base object", 1 == helper_alloc_count() - n);
	_it_should("set the base object's flags to 0", 0 == x_obj_flags(p_base));
	_it_should("set the base base object's gc pointer to NULL", NULL == x_obj_gc(p_base));

	/* Create an object */
	n = helper_alloc_count();
	p_obj[0] = x_obj_alloc(p_base, NULL, X_OBJ_FLAG_GC, 0);
	_it_should("allocate memory for the object", 1 == helper_alloc_count() - n);
	_it_should("set the object's flags to GC", X_OBJ_FLAG_GC == x_obj_flags(p_obj[0]));
	_it_should("set the object's gc pointer to NULL", NULL == x_obj_gc(p_obj[0]));
	_it_should("set the base object's gc pointer to the object", p_obj[0] == x_obj_gc(p_base));
	...
	_it_should("set the orphaned object's gc pointer to the first object", p_obj[0] == x_obj_gc(p_obj[1]));
	_it_should("set the base object's gc pointer to the orphaned object", p_obj[1] == x_obj_gc(p_base));

	n = helper_free_count();
	p_ret = x_gc_sweep(p_base, x_obj_gc(p_base), X_OBJ_FLAG_GC);
	_it_should("have freed the orphaned object's memory", 1 == helper_free_count() - n);
	_it_should("return the base object", p_base == p_ret);
	_it_should("set the base object's gc pointer to the first object", p_obj[0] == x_obj_gc(p_base));
	...
	n = helper_alloc_count();
	p_obj[0] = x_mkfsatom(p_base, X_OBJ_FLAG_RO|X_OBJ_FLAG_GC, 0);
	_it_should("allocate memory for the object", 1 == helper_alloc_count() - n);
	_it_should("set the object's flags to GC,RO", (X_OBJ_FLAG_RO|X_OBJ_FLAG_GC) == x_obj_flags(p_obj[0]));
	_it_should("set the object's gc pointer to NULL", NULL == x_obj_gc(p_obj[0]));
	_it_should("set the base object's gc pointer to the object", p_obj[0] == x_obj_gc(p_base));
	...
	/* Create an atom object */
	n = helper_alloc_count();
	p_obj[0] = x_mkfsatom(p_base, X_OBJ_FLAG_GC, 0);
	_it_should("allocate memory for the object", 1 == helper_alloc_count() - n);
	_it_should("set the object's flags to GC", X_OBJ_FLAG_GC == x_obj_flags(p_obj[0]));
	_it_should("set the object's gc pointer to NULL", NULL == x_obj_gc(p_obj[0]));
	_it_should("set the base object's gc pointer to the object", p_obj[0] == x_obj_gc(p_base));
	...
	_it_should("allocate memory for the orphaned object", 1 == helper_alloc_count() - n);
	_it_should("set the orphaned object's flags to 0", 0 == x_obj_flags(p_obj[1]));
	_it_should("set the orphaned object's gc pointer to the previous object", p_obj[0] == x_obj_gc(p_obj[1]));
	_it_should("set the base object's gc pointer to the object", p_obj[1] == x_obj_gc(p_base));
	...
	n = helper_alloc_count();
	s = (x_char_t *)"ATOMOWN";
	p_obj[0] = x_mkfsatomown(p_base, X_OBJ_FLAG_RO|X_OBJ_FLAG_GC, (x_obj_t *)x_lib_memdup(s, strlen((char *)s) + 1));
	_it_should("allocate memory for the object", 2 == helper_alloc_count() - n);
	_it_should("set the object's flags to OWN,RO,GC", (X_OBJ_FLAG_OWN|X_OBJ_FLAG_RO|X_OBJ_FLAG_GC) == x_obj_flags(p_obj[0]));
	_it_should("set the object's gc pointer to NULL", NULL == x_obj_gc(p_obj[0]));
	_it_should("set the base object's gc pointer to the object", p_obj[0] == x_obj_gc(p_base));
	...
	_it_should("return the base object",  p_base == p_ret);
	_it_should("cleared RO bit in the object's flags", (X_OBJ_FLAG_OWN|X_OBJ_FLAG_GC) == x_obj_flags(p_obj[0]));
	...
	n = helper_alloc_count();
	s = (x_char_t *)"ATOMOWN1";
	p_obj[0] = x_mkfsatomown(p_base, X_OBJ_FLAG_GC, (x_obj_t *)x_lib_memdup(s, strlen((char *)s) + 1));
	_it_should("allocate memory for the object and its resource", 2 == helper_alloc_count() - n);
	_it_should("set the object's flags to OWN,GC", (X_OBJ_FLAG_OWN|X_OBJ_FLAG_GC) == x_obj_flags(p_obj[0]));
	_it_should("set the object's gc pointer to NULL", NULL == x_obj_gc(p_obj[0]));
	_it_should("set the base object's gc pointer to the object", p_obj[0] == x_obj_gc(p_base));
	...
	/* Create an Atom object */
	n = helper_alloc_count();
	p_obj[0] = x_mkfsatom(p_base, X_OBJ_FLAG_RO|X_OBJ_FLAG_GC, "ATOM");
	_it_should("allocate memory for the atom object", 1 == helper_alloc_count() - n);
	_it_should("set the object's flags to RO,GC", (X_OBJ_FLAG_RO|X_OBJ_FLAG_GC) == x_obj_flags(p_obj[0]));
	_it_should("set the object's gc pointer to NULL", NULL == x_obj_gc(p_obj[0]));
	_it_should("set the base's gc pointer to the object", p_obj[0] == x_obj_gc(p_base));

	/* Create an Owner Atom object */
	n = helper_alloc_count();
	s = (x_char_t *)"ATOMOWN";
	p_obj[1] = x_mkfsatomown(p_base, X_OBJ_FLAG_RO|X_OBJ_FLAG_GC, x_lib_memdup(s, strlen((char *)s) + 1));
	_it_should("allocate memory for the atomown object", 2 == helper_alloc_count() - n);
	_it_should("set the object's flags to OWN,RO,GC", (X_OBJ_FLAG_OWN|X_OBJ_FLAG_RO|X_OBJ_FLAG_GC) == x_obj_flags(p_obj[1]));
	_it_should("set the object's gc pointer to the previous object", p_obj[0] == x_obj_gc(p_obj[1]));
	_it_should("set the base's gc pointer to the object", p_obj[1] == x_obj_gc(p_base));

	/* Create a Pair containing the two objects */
	n = helper_alloc_count();
	p_obj[2] = x_mkfspair(p_base, X_OBJ_FLAG_RO|X_OBJ_FLAG_GC, p_obj[0], p_obj[1]);
	_it_should("allocate memory for the pair object", 1 == helper_alloc_count() -n);
	_it_should("set the object's flags to GC,RO", (X_OBJ_FLAG_RO|X_OBJ_FLAG_GC) == x_obj_flags(p_obj[2]));
	_it_should("set the object's gc pointer to the previous object", p_obj[1] == x_obj_gc(p_obj[2]));
	_it_should("set the base's gc pointer to the object", p_obj[2] == x_obj_gc(p_base));

	n = helper_free_count();
	p_ret = x_gc_sweep(p_base, p_obj[2], X_OBJ_FLAG_RO);
	_it_should("not have freed the object's memory", 0 == helper_free_count() - n);
	_it_should("return the base object", p_base == p_ret);
	_it_should("cleared RO bit in the object's flags", X_OBJ_FLAG_GC == x_obj_flags(p_obj[2]));
	...
	_it_should("set the orphaned object's gc pointer to the pair object", p_obj[2] == x_obj_gc(p_obj[3]));
	_it_should("set the object's gc pointer to the orphaned object", p_obj[3] == x_obj_gc(p_base));

	n = helper_free_count();
	p_ret = x_gc_sweep(p_base, x_obj_gc(p_base), X_OBJ_FLAG_GC);
	_it_should("have freed the orphaned object's memory", 1 == helper_free_count() - n);
	_it_should("return the base object", p_base == p_ret);
	_it_should("set the object's gc pointer to the pair object", p_obj[2] == x_obj_gc(p_base));
